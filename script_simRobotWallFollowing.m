% script_simRobotWallFollowing.m
% This is a script that simulates a 2-wheel robot within a test course. The
% robot is equipped with one distance sensor whose reading emulates the
% behavior of an actual IR sensor (the GP2Y0A Sharp Sensor, specifically).
% The simulation loops through a time vector, and at each time lets the
% user calculate a turn and speed torque to send to the left and right
% motors respectively. It then sends these commands to a kinematic
% simulator that predicts the robot motion and the IR reading. If the robot
% hits a wall, the IR readings become NaN values which causes the code to
% stop. While driving, the position of the robot is tracked (the center
% point between the two dires). If the center is within a distance of
% user-definable "gates", the gates turn from red to green. If all gates
% turn green, the simulation ends and the run time is recorded.
% 
% The student objective is to program a wall-following algorithm such that
% all the gates are completed as fast as possible.
%
% Written by Sean Brennan, sbrennan@psu.edu on 11 April, 2020.
% 
